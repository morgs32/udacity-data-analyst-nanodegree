...
It looks like the distribution of fixed acidity is skewed positive.
...
Volatile acidity is also skewed positive by a relatively small number of outliers
...
This does not resemble a normal distribution. Apparently, very many wines have little to no citric acid at all. I suppose rice wines would fall into this category. It would be interesting to know if some grape wines can be made without any citric acid.
...
Again, the distribution is skewed positive, this time for residual sugar.
...
The far majority of wines have roughlty .85 chlorides according to this graph.
...
This distribution is skewed left with an even drop off after ~7 free sulfur dioxide, the maximum count of wines in this histogram.
...
This one is a bell curve around a mean of .9967
...
Another bell curve, this time around a mean of 3.311
...
Surprisingly, there were some wines with little or no alcohol content included in this study. Otherwise, the majority of wines contained between 9 and 11% alcohol.
...
As my last reviewer points out:
"Also, the quality distribution plot can be misleading.
This is because of the spaces between the bars.
It is unclear whether the bars represent discrete values(5, 6, 7...) or ranges(i.e., quality 5.0~5.9)"

```{r }

ggplot(aes_string(x = 'quality'), data = df) +
             geom_bar(stat="count")
summary(df['quality'])
